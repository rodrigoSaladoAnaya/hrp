// Instalador de HRP para Codex. Codex combina tres piezas:
...
function paths(root) {
  ...
  return {
    skillSource: path.join(pluginSource, "skills", "hrp"),
    marketplaceRoot,
...
export async function install(context) {
  ...
  if (!existsSync(serverEntry)) throw new Error(`Falta el build de HRP (${serverEntry}); ejecuta npm run build o ./scripts/update.sh`);

  const skillState = context.installSkill("codex");
  rep.action(`Skill hrp ${skillState}: ${context.skillState("codex")}`);
  ...
  const materializedSkill = path.join(cacheRoot, pluginVersion(resolved.pluginManifest), "skills", "hrp");
  ...
  const cacheSkillDir = path.join(cacheVersionDir, "skills", "hrp");
  ...
  if (existsSync(legacyWorkspaceSkill) && /HRP v[123]|Protocol [12]|v[123]\./.test(readFileSync(legacyWorkspaceSkill, "utf8"))) {
    rep.warn(`Existe una skill local HRP desactualizada en ${legacyWorkspaceSkill}; usa $hrp o actualízala.`);
  }
