...
3b. **Cuando el auditor del plan eres tú**, HRP te despierta con la señal accionable `plan`: obtén el paquete con `hrp graph review <run-id> --agent codex`, reporta con `hrp finding add <run-id> --title T --body B --severity S --scope plan --reviewer codex` y **cierra siempre tu pasada** con `hrp graph review done <run-id> --agent codex --findings N` (o la herramienta MCP `hrp_plan_pass`), también cuando el plan te parezca sano (`--findings 0`): sin ella el humano no puede aprobar. No inventes hallazgos para justificar la ronda.
